...
Oyunun temel mimarisi, CrazyGames platformundaki oyuncuların tüketim hızları ve davranış kalıpları analiz edilerek kurgulandı. Amacım sadece bir oyun yapmak değil; oyuncuyu sıkmayan, sürekli bir akış (flow state) içinde tutarak **oturumu terk etme oranını (churn rate) düşürmek** ve **oyunda kalma süresini (retention/playtime)** maksimize etmekti.
...
> 💡 **Tasarım Notu:** *Kasıtlı bir zorluk eğrisi (Pacing) tasarlandı. Oyuncuyu zorlayan kulelerin hemen ardından gelen görsel olarak tatmin edici bonus kuleler, oyuncuya 'nefes alma' ve başarma hissi (rewarding) vererek oyun döngüsünün (core loop) kırılmasını engeller.*

---

## ✨ Özel Geliştirici Araçları ve Sistem Mimari (Custom Tooling & Systems)

Bu projenin geliştirilme sürecinde beni en çok gururlandıran kısım, manuel bölüm tasarımını (level design) otomatize eden kendi üretim araçlarımı yazmış olmamdır.

### 1. Görsel Tabanlı Kule Üretim Aracı (Pixel-to-Tower Generator)
Bonus kulelerin tasarım sürecindeki zaman kaybını (bottleneck) ortadan kaldırmak için özel bir sistem geliştirdim. Bu sistem, **8x8 boyutundaki 2D Pixel Art görselleri tarayarak (Image Parsing) onları anında 3D oynanabilir kulelere dönüştürüyor.** 
* **Avantajı:** Bölüm tasarım süresini dakikalardan saniyelere indirdi ve oyuna hızlıca yeni içerik (content) eklememi sağlayan bir Pipeline (İş Hattı) oluşturdu.
...
Oyunun tekrar edilebilirliğini (Replayability) artırmak ve bellek kullanımını (Memory Usage) optimize etmek için yüzlerce farklı kule modellemek yerine **Modüler Prosedürel Mimari** kurdum. Sistem, kuleleri tek bir bütün olarak değil; **Base (Zemin), Mid (Gövde) ve Top (Çatı)** olmak üzere 3 katmana ayırır.
